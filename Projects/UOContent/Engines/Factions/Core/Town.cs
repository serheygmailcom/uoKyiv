...
public abstract class Town : IComparable<Town>
{
    ...
    {
        var defs = Owner?.Definition.Guards ?? [];
    ...
    public bool UnregisterVendor(BaseFactionVendor vendor) =>
        vendor != null && (FindVendorList(vendor.GetType())?.Vendors.Remove(vendor) ?? false);
    ...
    {
        // Looks useless, but is used to set the PlayerState for these roles
        // TODO: When this moves to a deserialization pattern, put it in AfterDeserialization
        for (var i = 0; i < Towns.Count; ++i)
        {
            var town = Towns[i];
            town.Sheriff = town.Sheriff;
            town.Finance = town.Finance;
        }
    }
